...
describe('simple household budget',()=>{
 it.each(['confirmed','estimated','unknown','obligation'] as const)('counts the full expense regardless of legacy %s allocation',loanBasis=>{
  const rows=[tx({loanBasis})],total=monthlyTotals(rows,2026,8,'UAH')
  expect(total.expense).toBe(9060)
  const quality=reportQuality(rows,'UAH','2026-09')
  expect(quality.provisional).toBe(false)
  expect(quality.incomplete).toBe(false)
  expect(quality.unallocatedAmount).toBe(0)
  expect(quality.estimated).toEqual([])
 ...
 it('uses the same recognized cost in categories and totals',()=>{
  ...
  const categories=spendingByCategory(rows,[],2026,8,'UAH')
  expect(categories.find(c=>c.categoryId==='loan-category')?.total).toBe(9060)
 ...
 it('keeps monthly and annual results consistent',()=>{
  ...
  expect(year[8].income).toBe(20000);expect(year[8].expense).toBe(9060);expect(year[8].net).toBe(10940)
  expect(year.reduce((n,m)=>n+m.expense,0)).toBe(9060)
 })
 it('preserves debt allocation as details without splitting the transaction',()=>{
  ...
  expect(monthlyTotals([payment],2026,8,'UAH').expense).toBe(9060)
 })
})

it('keeps legacy component rounding gaps out of the household expense total',()=>{
 const rows=[tx()]
 const quality=reportQuality(rows,'UAH','2026-09')
 expect(quality.componentGaps).toEqual([])
 expect(quality.recognizedExpense).toBe(9060)
 expect(quality.incomplete).toBe(false)
})