...
export function AccountPreferences(){
 ...
 return <form className="space-y-4" onSubmit={async e=>{e.preventDefault();if(save.isPending)return;try{await save.mutateAsync({defaultAccountId:paymentAccounts.some(a=>a.id===main)?main:null,favoriteAccountIds:favorites.filter(id=>paymentAccounts.some(a=>a.id===id))});toast.success('Ваші рахунки налаштовано')}catch(e){toast.error('Не вдалося зберегти',e)}}}>
 <p className="text-sm text-secondary">Ці налаштування діють лише для вас, на всіх ваших пристроях.</p><Select label="Мій основний рахунок" value={paymentAccounts.some(a=>a.id===main)?main:''} onChange={e=>setMain(e.target.value)} options={[{value:'',label:'Без основного рахунку'},...paymentAccounts.map(a=>({value:a.id,label:a.name+' · '+accountOwner(a,members)}))]}/><fieldset className="space-y-1"><legend className="mb-2 text-sm font-medium">Обрані для швидкого доступу</legend>{paymentAccounts.map(a=><Checkbox className="flex w-full" key={a.id} checked={favorites.includes(a.id)} onChange={e=>setFavorites(v=>e.target.checked?[...v,a.id]:v.filter(id=>id!==a.id))}>{a.name}<span className="block text-xs text-muted">{accountOwner(a,members)}</span></Checkbox>)}</fieldset><p className="text-xs text-muted">Основний підставляється автоматично. Обрані показуються окремими кнопками. Усі інші рахунки залишаються в загальному списку.</p><Button type="submit" disabled={!settings||save.isPending}>Зберегти мої рахунки</Button></form>
}